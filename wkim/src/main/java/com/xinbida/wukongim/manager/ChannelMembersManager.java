...
public class ChannelMembersManager extends BaseManager {
    ...
    public synchronized void save(WKChannelMember member) {
        ...
        // - row 不存在 → insert（行为不变）；
        // - row 存在 → update，且 {@code getContentValuesWithChannelMember} 在 extraMap==null 时
        //   不写入 extra 列 → SQL UPDATE 不触碰 extra → 既有 extras 保留。
        ChannelMembersDbManager.getInstance().insertOrUpdate(member);
        if (handelType == 0) {
