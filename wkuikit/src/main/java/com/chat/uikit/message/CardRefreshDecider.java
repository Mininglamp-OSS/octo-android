 ...
 * </ul>
 * 注意：无 content_edit 的补拉（纯展示卡、以及流式卡首帧到达前）一律当"非信号"不计数（P1-3），
 * 不走此计数收敛，改由 {@link MsgModel} 的墙钟窗口（MAX_COMPENSATION_WINDOW_MS）兜底注销；此计数只
 * 服务"已有真实 content_edit 但 bot 卡在同一非 transient 帧"这一窄场景。</p>
 *
