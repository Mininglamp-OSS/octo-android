...
public class MsgDbManager {
    ...
    public void queryOrSyncHistoryMessages(String channelId, byte channelType, long oldestOrderSeq, boolean contain, int pullMode, int limit, final IGetOrSyncHistoryMsgBack iGetOrSyncHistoryMsgBack) {
        ...
        if (!list.isEmpty() && previewPosted.putIfAbsent(previewKey, Boolean.TRUE) == null) {
            // Snapshot the current local list; the recursion below may mutate
            // `list` via subsequent queries when sync writes to the DB.
            final List<WKMsg> preview = new ArrayList<>(list);
            new Handler(Looper.getMainLooper()).post(() -> iGetOrSyncHistoryMsgBack.onResult(preview));
        }
//        if (more == 0) {
